...
# print_drivetrain_weights.py 

# Created: SUAVE team
# Updated: Kent Hamilton, Jan 2020
...
# ----------------------------------------------------------------------
#  Print output file with weight breakdown of a turboelectric powertrain
# ----------------------------------------------------------------------
## @ingroup Input_Output-Results
def print_drivetrain_weight_breakdown(config,filename = 'drivetrain_weight_breakdown.dat'):
    """This creates a file showing the drivetrain weight information.

    Assumptions:
    One turboelectric propulsor (can be multiple engines and multiple powersupplies) with 'turbofan' tag.
    ...
    fid.write('Output file with powertrain weight breakdown\n\n') #Start output printing
    ...
    fid.write(' POWERTRAIN WEIGHT BREAKDOWN  \n')       
    for tag,value in weight_breakdown.systems_breakdown.items():
        tag = tag.replace('_',' ')
        string = ' ' + tag[0].upper() + tag[1:] + ' '
        string = string.ljust(33,'.') + ' :' 
