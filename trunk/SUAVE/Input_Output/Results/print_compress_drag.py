...
def print_compress_drag(vehicle,analyses,filename = 'compress_drag.dat'):
    ...
    fid.write(np.insert(np.transpose(list(map('M={:5.3f} | '.format,(mach_vec)))),0,'  CL   |  '))
    fid.write('\n')
    ...
    for idcl, cl in enumerate(cl_vec):
        ...
        fid.write(np.insert((np.transpose(list(map('{:7.5f} | '.format,(cd_compress_tot[:,idcl]))))),0,' {:5.3f} |  '.format(cl)))
        fid.write('\n')
    fid.write( 119*'-' )
    ...
    for wing in vehicle.wings: 
        ...
        fid.write('\n')
        for idcl, cl in enumerate(cl_vec):
            fid.write(np.insert((np.transpose(list(map('{:7.5f} | '.format,(cd_compress[wing.tag][:,idcl]))))),0,' {:5.3f} |  '.format(cl)))
            fid.write('\n')
        fid.write(119*'-')
    # close file
    fid.close
