...
def update_wake_position_time_marching(wake, rotor, conditions):  
    ...
    for i in range(nts):
        ...
        #--------------------------------------------------------------------------------------------  
        new_wake_reduced = reduce_wake(new_wake) # only consisting of panels that have already been shed
        WD_network = Data()
        ...
        new_wake.reshaped_wake.GAMMA[:,:,:,:,nts-1-i] = WD.reshaped_wake.GAMMA[rolled_start,:,:,:,1]          
...
def generate_temp_wake(WD):
    ...
    temp_wake.reshaped_wake.GAMMA = copy.deepcopy(WD.reshaped_wake.GAMMA)    
    temp_wake.GAMMA = copy.deepcopy(WD.GAMMA)
