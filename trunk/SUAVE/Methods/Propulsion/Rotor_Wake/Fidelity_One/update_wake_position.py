...
## @ingroup Methods-Propulsion-Rotor_Wake-Fidelity_One
def update_wake_position(wake, rotor, VD=None):  
    ...
    #--------------------------------------------------------------------------------------------  
    fun_V_induced, interpolatedBoxData = compute_interpolated_velocity_field(WD, VD)
    
    #--------------------------------------------------------------------------------------------    
    # Step 2: Compute the position of new trailing edge panel under all velocity influences
    #--------------------------------------------------------------------------------------------
    new_wake = generate_temp_wake(WD)
    for i in range(nts):
        na_start = i % Na
        if i == nts-1:
            ...
            # shed a row of trailing edge nodes at this time step
