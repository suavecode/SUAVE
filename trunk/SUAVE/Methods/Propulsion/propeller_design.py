...
def propeller_design(prop,number_of_stations=20,number_of_airfoil_section_points = 200):
    ...
    t_c    = np.zeros(N)   
    if airfoil_flag:      
        t_max                 = np.take(airfoil_data.max_thickness,a_loc,axis=0)*c 
        t_c                   = np.take(airfoil_data.thickness_to_chord,a_loc,axis=0)  
    else:     
