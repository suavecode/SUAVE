...
def compute_component_centers_of_gravity(vehicle):
    ...
    mac_le_offset                              = .8*np.sin(wing.sweep.leading_edge)*span_location_mac  
    ...
    h_tail_35_percent_semi_span_offset         =.8*np.sin(h_tail.sweep.quarter_chord)*.35*.5*h_tail.spans.projected             
    v_tail_35_percent_semi_span_offset         =.8*np.sin(v_tail.sweep.quarter_chord)*.35*.5*v_tail.spans.projected
