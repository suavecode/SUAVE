...
## @ingroup Methods-Geometry-Two_Dimensional-Cross_Section-Airfoil
def compute_airfoil_polars(a_geo,a_polar,npoints = 200 ,use_pre_stall_data=True, use_uq=False): #True):
    ...
    for i in range(num_airfoils):
        ...
        for j in range(len(a_polar[i])):
            ...
            # max lift coefficent and associated aoa (range > 0 to avoid bad negative xfoil data)
            ...
            CD1max  = np.max(airfoil_cd[airfoil_aoa > 0 * Units.deg]) 
            idx_aoa_max_prestall_cd = np.where(airfoil_cd == CD1max)[0][0]
            ACD1   = airfoil_aoa[idx_aoa_max_prestall_cd] * Units.degrees     
            
            # Find the point of lowest drag and the CD (range within +/-6deg to avoid bad data)
            idx_CD_min = np.where(airfoil_cd==min(airfoil_cd[abs(airfoil_aoa) < 6 * Units.deg]))[0][0]
