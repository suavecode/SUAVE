...
def heads_method(theta_0, del_star_0, L, Re_L, x_i, Ve_i, dVe_i):
    ...
    x = np.zeros((n, 1))
    y = np.zeros((n,2))
    [x, y] = ode45(@(x,y) odefcn(x,y,Re_L/L, x_i, Ve_i, dVe_i), xspan, y0)
    theta              = y[:,1] 
    Ve_theta_H1  = y[:,2]
    H1          = np.zeros_like(x)
    H           = np.zeros_like(x)
...
def odefcn(x,y, ReL_div_L, x_i, Ve_i, dVe_i):
    dydx = np.zeros(2,1)
    theta = y[0]
    Ve_theta_H1 = y(2)
    if theta == 0:
    ...
    cf = getcf(Re_theta,H)
    dydx[0]= 0.5*cf-theta/getVe(x,x_i,Ve_i)*(2+H)*getdVe(x, x_i, dVe_i)
    dydx(2) = getVe(x,x_i,Ve_i)*0.0306*(H1 - 3)**-0.6169
