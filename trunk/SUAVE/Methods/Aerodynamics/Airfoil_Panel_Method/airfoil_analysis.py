...
def airfoil_analysis(airfoil_geometry,alpha,Re_L,npanel = 100 ,airfoil_stations = [0],
    ...
    aoas            = list(np.repeat(np.arange(ndim),ncpts))
    res             = list(np.tile(np.arange(ncpts),ndim) )
    ...
    BOT_T_RESULTS  = thwaites_method(npanel,ndim,ncpts, L_BOT , RE_L_VALS, X_BOT, VE_BOT, DVE_BOT,tolerance,
    ...
    X_TR_BOT          = X_T_BOT[transition_panel,aoas,res].reshape(ndim,ncpts)
    DELTA_STAR_TR_BOT = DELTA_STAR_T_BOT[transition_panel,aoas,res].reshape(ndim,ncpts)
    THETA_TR_BOT      = THETA_T_BOT[transition_panel,aoas,res].reshape(ndim,ncpts)    
    DELTA_TR_BOT      = DELTA_T_BOT[transition_panel,aoas,res].reshape(ndim,ncpts) 
    ...
    BOT_H_RESULTS     = heads_method(npanel,ndim,ncpts,DELTA_TR_BOT ,THETA_TR_BOT , DELTA_STAR_TR_BOT,
    ...
    X_BOT_SURF           = X_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    Y_BOT_SURF           = Y_BOT 
    THETA_BOT_SURF       = THETA_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    DELTA_STAR_BOT_SURF  = DELTA_STAR_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    H_BOT_SURF           = H_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')
    CF_BOT_SURF          = CF_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')
    RE_THETA_BOT_SURF    = RE_THETA_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    RE_X_BOT_SURF        = RE_X_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    DELTA_BOT_SURF       = DELTA_BOT_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')          
    ...
    # laminar boundary layer properties using thwaites method 
    TOP_T_RESULTS    = thwaites_method(npanel,ndim,ncpts, L_TOP , RE_L_VALS,X_TOP,VE_TOP, DVE_TOP,tolerance,
                                     THETA_0=initial_momentum_thickness) 
    ...
    X_TR_TOP          = X_T_TOP[transition_panel,aoas,res].reshape(ndim,ncpts)
    DELTA_STAR_TR_TOP = DELTA_STAR_T_TOP[transition_panel,aoas,res].reshape(ndim,ncpts)
    THETA_TR_TOP      = THETA_T_TOP[transition_panel,aoas,res].reshape(ndim,ncpts)    
    DELTA_TR_TOP      = DELTA_T_TOP[transition_panel,aoas,res].reshape(ndim,ncpts) 
   
    TURBULENT_SURF    = L_TOP.data  - X_TR_TOP
    TURBULENT_COORD   = np.ma.masked_less( X_TOP.data  - X_TR_TOP,0)

    # turbulent boundary layer properties using heads method 
    TOP_H_RESULTS     = heads_method(npanel,ndim,ncpts,DELTA_TR_TOP ,THETA_TR_TOP , DELTA_STAR_TR_TOP,
    ...
    X_TOP_SURF           = X_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F') 
    Y_TOP_SURF           = Y_TOP 
    THETA_TOP_SURF       = THETA_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')   
    DELTA_STAR_TOP_SURF  = DELTA_STAR_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F') 
    H_TOP_SURF           = H_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')
    CF_TOP_SURF          = CF_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')
    RE_THETA_TOP_SURF    = RE_THETA_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    RE_X_TOP_SURF        = RE_X_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')  
    DELTA_TOP_SURF       = DELTA_TOP_SURF_2.reshape((npanel,ndim,ncpts),order = 'F')          

    
    # ------------------------------------------------------------------------------------------------------
    # concatenate lower and upper surfaces   
    # ------------------------------------------------------------------------------------------------------ 
    X_PANEL    = concatenate_surfaces(X_BOT,X_TOP,X_BOT_SURF,X_TOP_SURF,npanel,ndim,ncpts)
    Y_PANEL    = concatenate_surfaces(X_BOT,X_TOP,Y_BOT_SURF,Y_TOP_SURF,npanel,ndim,ncpts)
    THETA      = concatenate_surfaces(X_BOT,X_TOP,THETA_BOT_SURF,THETA_TOP_SURF,npanel,ndim,ncpts)
    DELTA_STAR = concatenate_surfaces(X_BOT,X_TOP,DELTA_STAR_BOT_SURF,DELTA_STAR_TOP_SURF,npanel,ndim,ncpts) 
    H          = concatenate_surfaces(X_BOT,X_TOP,H_BOT_SURF,H_TOP_SURF,npanel,ndim,ncpts)  
    CF         = concatenate_surfaces(X_BOT,X_TOP,CF_BOT_SURF,CF_TOP_SURF,npanel,ndim,ncpts) 
    RE_THETA   = concatenate_surfaces(X_BOT,X_TOP,RE_THETA_BOT_SURF,RE_THETA_TOP_SURF,npanel,ndim,ncpts)  
    RE_X       = concatenate_surfaces(X_BOT,X_TOP,RE_X_BOT_SURF,RE_X_TOP_SURF,npanel,ndim,ncpts) 
    DELTA      = concatenate_surfaces(X_BOT,X_TOP,DELTA_BOT_SURF,DELTA_TOP_SURF,npanel,ndim,ncpts)   
    ...
    VE         = VE_VALS_2.reshape((npanel,ndim,ncpts),order = 'F') 
    DVE        = DVE_VALS_2.reshape((npanel,ndim,ncpts),order = 'F')  
    ...
    CP_BL        = CP_BL_VALS_2.reshape((npanel,ndim,ncpts),order = 'F')     
...
def concatenate_surfaces(X_BOT,X_TOP,FUNC_BOT_SURF,FUNC_TOP_SURF,npanel,ndim,ncpts): 
    ...
    ndim         - number of angle of attacks                                    [unitless]
    ...
    N/A  
    '''  
    FUNC = np.zeros((npanel,ndim,ncpts)) 
    N_ALPHA = 1 
