...
def airfoil_analysis(airfoil_geometry,alpha,Re_L,npanel = 100 , batch_analysis = True, airfoil_stations = [0],
    ...
    airfoil_properties = Data(
        ...
        Cd         = AERO_RES_BL.Cd,
        Cm         = AERO_RES_BL.Cm,  
        normals    = np.swapaxes(np.swapaxes(normals,0,2),1,3),
        x          = np.swapaxes(np.swapaxes(X,0,1),1,2),
        y          = np.swapaxes(np.swapaxes(Y,0,1),1,2),        
        x_bl       = np.swapaxes(np.swapaxes(X_BL,0,1),1,2),
        y_bl       = np.swapaxes(np.swapaxes(Y_BL,0,1),1,2),
        Cp         = np.swapaxes(np.swapaxes(CP_BL,0,1),1,2),         
        Ue_Vinf    = np.swapaxes(np.swapaxes(VE,0,1),1,2),         
        dVe        = np.swapaxes(np.swapaxes(DVE,0,1),1,2),   
        theta      = np.swapaxes(np.swapaxes(THETA,0,1),1,2),      
        delta_star = np.swapaxes(np.swapaxes(DELTA_STAR,0,1),1,2), 
        delta      = np.swapaxes(np.swapaxes(DELTA,0,1),1,2),
        Re_theta   = np.swapaxes(np.swapaxes(RE_THETA,0,1),1,2),
        Re_x       = np.swapaxes(np.swapaxes(RE_X,0,1),1,2),
        H          = np.swapaxes(np.swapaxes(H,0,1),1,2),               
        Cf         = np.swapaxes(np.swapaxes(CF,0,1),1,2),          
        )  
