...
from SUAVE.Methods.Aerodynamics.Common.Fidelity_Zero.Lift.make_VLM_wings import convert_to_segmented_wing
    ...
    for i_segs in range(n_segments):
        ...
        if (i_segs == n_segments-1):
            ...
            if getattr(suave_wing,'control_surfaces',False):    
                dihedral_ob   = segments[i_segs].dihedral_outboard 
                section_spans = []
                for cs in suave_wing.control_surfaces:     
                    ...
                    if (control_surface_start < semispan*segments[i_segs+1].percent_span_location) and (control_surface_start >= semispan*segments[i_segs].percent_span_location) : 
                        section_spans.append(control_surface_start) 
                    if (control_surface_end  < semispan*segments[i_segs+1].percent_span_location) and (control_surface_end  >= semispan*segments[i_segs].percent_span_location):                         
                ...
                for section_count in range(num_sections):        
                    ...
                    root_section_twist        = segments[i_segs].twist/Units.degrees 
                    tip_section_twist         = root_chord*segments[i_segs+1].twist/Units.degrees  
