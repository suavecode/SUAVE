...
# ----------------------------------------------------------------------
## @ingroup Methods-Aerodynamics-Common-Fidelity_Zero-Lift
def generate_nacelle_vortex_distribution(VD,nac,n_cw,n_sw,precision,model_nacelle=False):
    ...
    vec2               = [1  ,1.57 , 3.2,  8]
    x                  = np.linspace(0,1,4)
    nac_nose_curvature =  np.interp(np.interp(nac.fineness.nose,vec2,x), x , vec1)
    nac_tail_curvature =  np.interp(np.interp(nac.fineness.tail,vec2,x), x , vec1) 

    # --TO DO-- model nacelage segments if defined, else use the following code
    
    # Horizontal Sections of nacelage
    nhs        = Data()        
    ...
    for i in range(n_sw+1): 
        nhs_cabin_length  = nac.lengths.total - (nac.lengths.nose + nac.lengths.tail)
        nhs.nose_length   = ((1 - ((abs(h_array[i]/semispan_h))**nac_nose_curvature ))**(1/nac_nose_curvature))*nac.lengths.nose
        nhs.tail_length   = ((1 - ((abs(h_array[i]/semispan_h))**nac_tail_curvature ))**(1/nac_tail_curvature))*nac.lengths.tail
        nhs.nose_origin   = nac.lengths.nose - nhs.nose_length 
        nhs.origin[i][:]  = np.array([origin[0] + nhs.nose_origin , origin[1] + h_array[i], origin[2]])
        nhs.chord[i]      = nhs_cabin_length + nhs.nose_length + nhs.tail_length          

        nvs_cabin_length  = nac.lengths.total - (nac.lengths.nose + nac.lengths.tail)
        nvs.nose_length   = ((1 - ((abs(v_array[i]/semispan_v))**nac_nose_curvature ))**(1/nac_nose_curvature))*nac.lengths.nose
        nvs.tail_length   = ((1 - ((abs(v_array[i]/semispan_v))**nac_tail_curvature ))**(1/nac_tail_curvature))*nac.lengths.tail
        nvs.nose_origin   = nac.lengths.nose - nvs.nose_length 
        nvs.origin[i][:]  = np.array([origin[0] + nvs.nose_origin , origin[1] , origin[2]+  v_array[i]])
        nvs.chord[i]      = nvs_cabin_length + nvs.nose_length + nvs.tail_length
    ...
    # xyz positions for the right side of this nacelage's outermost panels
