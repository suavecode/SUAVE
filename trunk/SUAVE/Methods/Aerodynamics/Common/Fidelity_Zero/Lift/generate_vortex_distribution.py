...
def generate_vortex_distribution(geometry,settings):
   ...
   XA2 |_|________________________|_|XB2
    ...
    VD.n_cp     = int(np.array(VD.n_cp))
    VD.total_sw = int(np.array(VD.total_sw))
...
def generate_wing_vortex_distribution(VD,wing,n_cw,n_sw,spc,precision):
    ...
    for sym_sign in signs[symmetry_mask]:
        ...
        xah   = jnp.zeros(n_cw*n_sw,dtype=precision)
        yah   = jnp.zeros(n_cw*n_sw,dtype=precision)
        zah   = jnp.zeros(n_cw*n_sw,dtype=precision)
        xbh   = jnp.zeros(n_cw*n_sw,dtype=precision)
        ybh   = jnp.zeros(n_cw*n_sw,dtype=precision)
        zbh   = jnp.zeros(n_cw*n_sw,dtype=precision)    
        xch   = jnp.zeros(n_cw*n_sw,dtype=precision)
        ych   = jnp.zeros(n_cw*n_sw,dtype=precision)
        zch   = jnp.zeros(n_cw*n_sw,dtype=precision)    
        xa1   = jnp.zeros(n_cw*n_sw,dtype=precision)
        ya1   = jnp.zeros(n_cw*n_sw,dtype=precision)
        za1   = jnp.zeros(n_cw*n_sw,dtype=precision)
        xa2   = jnp.zeros(n_cw*n_sw,dtype=precision)
        ya2   = jnp.zeros(n_cw*n_sw,dtype=precision)
        za2   = jnp.zeros(n_cw*n_sw,dtype=precision)    
        xb1   = jnp.zeros(n_cw*n_sw,dtype=precision)
        yb1   = jnp.zeros(n_cw*n_sw,dtype=precision)
        zb1   = jnp.zeros(n_cw*n_sw,dtype=precision)
        xb2   = jnp.zeros(n_cw*n_sw,dtype=precision) 
        yb2   = jnp.zeros(n_cw*n_sw,dtype=precision) 
        zb2   = jnp.zeros(n_cw*n_sw,dtype=precision)    
        xac   = jnp.zeros(n_cw*n_sw,dtype=precision)
        yac   = jnp.zeros(n_cw*n_sw,dtype=precision)
        zac   = jnp.zeros(n_cw*n_sw,dtype=precision)    
        xbc   = jnp.zeros(n_cw*n_sw,dtype=precision)
        ybc   = jnp.zeros(n_cw*n_sw,dtype=precision)
        zbc   = jnp.zeros(n_cw*n_sw,dtype=precision)     
        xc    = jnp.zeros(n_cw*n_sw,dtype=precision) 
        yc    = jnp.zeros(n_cw*n_sw,dtype=precision) 
        zc    = jnp.zeros(n_cw*n_sw,dtype=precision) 
        x     = jnp.zeros((n_cw+1)*(n_sw+1),dtype=precision) # may have to change to make space for split if control surfaces are allowed to have more than two Segments
        y     = jnp.zeros((n_cw+1)*(n_sw+1),dtype=precision) 
        z     = jnp.zeros((n_cw+1)*(n_sw+1),dtype=precision)         
        cs_w  = jnp.zeros(n_sw,dtype=precision)
