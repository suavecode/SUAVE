...
def VLM(conditions,settings,geometry):
    ...
    u_n_w        = np.array(onp.array_split(u,n_w,axis=1))
    u_n_w_sw     = np.array(onp.array_split(u,n_w*n_sw,axis=1))
    w_ind_n_w    = np.array(onp.array_split(w_ind,n_w,axis=1))
    w_ind_n_w_sw = np.array(onp.array_split(w_ind,n_w*n_sw,axis=1))    
    gamma_n_w    = np.array(onp.array_split(gamma,n_w,axis=1))
    gamma_n_w_sw = np.array(onp.array_split(gamma,n_w*n_sw,axis=1))
    Del_Y_n_w    = np.array(onp.array_split(Del_Y,n_w,axis=1))
    Del_Y_n_w_sw = np.array(onp.array_split(Del_Y,n_w*n_sw,axis=1))
