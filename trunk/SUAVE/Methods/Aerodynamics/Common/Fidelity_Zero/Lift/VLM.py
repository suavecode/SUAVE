...
def VLM(conditions,configuration,geometry):
            ...
            if n_segments>0:
                ...
                for idx_y in range(n_sw):                    
                    for idx_x in range(n_cw):  
                        ...
                        xi_ah = eta_a*np.tan(sweep) + delta_x_a*idx_x + delta_x_a*0.25 # x coordinate of top left corner of bound vortex 
                        ...
                        xi_bh = eta_b*np.tan(sweep) + delta_x_b*idx_x + delta_x_b*0.25 # x coordinate of top right corner of bound vortex 
                        ...
                        wing_camber_c  = wing_camber*wing_chord_section
                        ...
                        z_c_a1 = np.interp((idx_x*delta_x_a)                 ,section_x_coord_a,wing_camber_a)
                        z_c_ah = np.interp((idx_x*delta_x_a + delta_x_a*0.25),section_x_coord_a,wing_camber_a) 
                        z_c_a2 = np.interp(((idx_x+1)*delta_x_a)             ,section_x_coord_a,wing_camber_a) 
                        z_c_b1 = np.interp((idx_x*delta_x_b)                 ,section_x_coord_b,wing_camber_b) 
                        z_c_bh = np.interp((idx_x*delta_x_b + delta_x_b*0.25),section_x_coord_b,wing_camber_b)  
                        z_c_b2 = np.interp(((idx_x+1)*delta_x_b)             ,section_x_coord_b,wing_camber_b) 
                        z_c    = np.interp((idx_x*delta_x + delta_x*0.75)    ,section_x_coord,wing_camber_c)
                        z_c_ch = np.interp((idx_x*delta_x + delta_x*0.25)    ,section_x_coord,wing_camber_c)
                        
                        zeta_a1 = eta_a*np.tan(dihedral)  + z_c_a1  # z coordinate of top left corner of panel
                        zeta_ah = eta_a*np.tan(dihedral)  + z_c_ah  # z coordinate of left quarter-chord of panel
                        ...
                        zeta_bh = eta_b*np.tan(dihedral)  + z_c_bh  # z coordinate of right quarter-chord of panel  
                        ...
                        xi_prime_a1  = xi_LE_a + np.cos(section_twist_a)*(xi_a1-xi_LE_a) + np.sin(section_twist_a)*(zeta_a1-zeta_LE_a)   # x coordinate transformation of top left corner
                        xi_prime_ah  = xi_LE_a + np.cos(section_twist_a)*(xi_ah-xi_LE_a) + np.sin(section_twist_a)*(zeta_ah-zeta_LE_a)   # x coordinate transformation of bound vortex left corner 
                        xi_prime_a2  = xi_LE_a + np.cos(section_twist_a)*(xi_a2-xi_LE_a) + np.sin(section_twist_a)*(zeta_a2-zeta_LE_a)   # x coordinate transformation of bottom left corner
                        xi_prime_b1  = xi_LE_b + np.cos(section_twist_b)*(xi_b1-xi_LE_b) + np.sin(section_twist_b)*(zeta_b1-zeta_LE_b)   # x coordinate transformation of top right corner
                        xi_prime_bh  = xi_LE_b + np.cos(section_twist_b)*(xi_bh-xi_LE_b) + np.sin(section_twist_b)*(zeta_bh-zeta_LE_b)   # x coordinate transformation of bound vortex right corner  
                        xi_prime_b2  = xi_LE_b + np.cos(section_twist_b)*(xi_b2-xi_LE_b) + np.sin(section_twist_b)*(zeta_b2-zeta_LE_b)   # x coordinate transformation of botton right corner 
                        xi_prime     = xi_LE   + np.cos(section_twist)*(xi-xi_LE)        + np.sin(section_twist)*(zeta-zeta_LE)                   # x coordinate transformation of control point
                        xi_prime_ch  = xi_LE   + np.cos(section_twist)*(xi_ch-xi_LE)     + np.sin(section_twist)*(zeta_ch-zeta_LE)             # x coordinate transformation of center of horeshoe vortex 
                        
                        zeta_prime_a1  = zeta_LE_a - np.sin(section_twist_a)*(xi_a1-xi_LE_a) + np.cos(section_twist_a)*(zeta_a1-zeta_LE_a) # z coordinate transformation of top left corner
                        zeta_prime_ah  = zeta_LE_a - np.sin(section_twist_a)*(xi_ah-xi_LE_a) + np.cos(section_twist_a)*(zeta_ah-zeta_LE_a) # z coordinate transformation of bound vortex left corner 
                        zeta_prime_a2  = zeta_LE_a - np.sin(section_twist_a)*(xi_a2-xi_LE_a) + np.cos(section_twist_a)*(zeta_a2-zeta_LE_a) # z coordinate transformation of bottom left corner
                        zeta_prime_b1  = zeta_LE_b - np.sin(section_twist_b)*(xi_b1-xi_LE_b) + np.cos(section_twist_b)*(zeta_b1-zeta_LE_b) # z coordinate transformation of top right corner 
                        zeta_prime_bh  = zeta_LE_b - np.sin(section_twist_b)*(xi_bh-xi_LE_b) + np.cos(section_twist_b)*(zeta_bh-zeta_LE_b) # z coordinate transformation of bound vortex right corner  
                        zeta_prime_b2  = zeta_LE_b - np.sin(section_twist_b)*(xi_b2-xi_LE_b) + np.cos(section_twist_b)*(zeta_b2-zeta_LE_b) # z coordinate transformation of botton right corner 
                        zeta_prime     = zeta_LE   - np.sin(section_twist)  *(xi-xi_LE)      + np.cos(section_twist)  *(zeta-zeta_LE)      # z coordinate transformation of control point
    ...
    #axes3.plot3D(XA1,YA,ZA1,'bo') 
    #axes3.plot3D(XA2,YA,ZA2,'rx')
    #axes3.plot3D(XB1,YB,ZB1,'g1') 
    #axes3.plot3D(XB2,YB,ZB2,'yv')  
    #axes3.plot3D(XC ,YC,ZC ,'ks')
    #axes3.plot3D(XAH,YA,ZAH,'cD')
    #axes3.plot3D(XBH,YB,ZBH,'k+')    
    #axes3.set_zlabel('height')
    #axes3.set_xlabel('chord ')
    #axes3.set_ylabel('span ')
    #axes3.grid(True)
    #plt.show()
    ...
    phi   = np.arctan((ZB2 - ZA2)/(YB - YA))
    delta = np.arctan((ZA2 - ZA1)/(XA2 - XA1))
    ...
    gamma = np.linalg.solve(A.T,RHS.T).T
    ...
    Del_Y = YB - YA
    for j in range(n_w):
        CL_wing[j] = 2*np.sum(gamma[0][j*n_cppw:(j+1)*n_cppw] * Del_Y[j*n_cppw:(j+1)*n_cppw])/wing_areas[j]  # wing lift coefficient
        CDi_wing[j] = -2*np.sum(gamma[0][j*n_cppw:(j+1)*n_cppw] * Del_Y[j*n_cppw:(j+1)*n_cppw])/wing_areas[j] # wing induced drag coefficient
        for k in range(n_sw):   
            Cl_wing[j,k]  = 2*np.sum(gamma[0][i*n_cw:(i+1)*n_cw] * Del_Y[i*n_cw:(i+1)*n_cw])/CS[i] # sectional lift coefficients 
            Cdi_wing[j,k] = -2*np.sum(gamma[0][i*n_cw:(i+1)*n_cw] * Del_Y[i*n_cw:(i+1)*n_cw]*w[0][i*n_cw:(i+1)*n_cw])/CS[i] # sectional induced drag coefficients
            i += 1 
 
    CL  = np.sum(gamma[0]*Del_Y)/(Sref)                   # vehicle lift coefficient
    CDi = -np.sum(gamma[0]*Del_Y*w[0])/(Sref)                  # vehicle lift coefficient
    CM  = np.sum(gamma[0]*Del_Y*(X_M - XCH))/(Sref*c_bar) # vehicle lift coefficient
