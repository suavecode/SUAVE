...
## @ingroupMethods-Noise-Fidelity_One-Noise_Tools 
def compute_point_source_coordinates(i,p_idx,AoA,thrust_angle,mls,prop_origin):  
    ...
    rotation_1        = np.zeros((num_mic,4,4))
    ...
    I                    = np.swapaxes(I,1,2)
    translation_1        = np.repeat(I,num_mic, axis = 0)
    ...
    rotation_2        = np.zeros((num_mic,4,4))
    ...
    I                  = np.atleast_3d(np.eye(4)).T
    I                  = np.swapaxes(I,1,2)
    translate_2        = np.repeat(I,num_mic, axis = 0)     
    translate_2[:,0,3] = mls[i,:,0]  
    translate_2[:,1,3] = mls[i,:,1]   
    translate_2[:,2,3] = mls[i,:,2] 
    
    I0    =  np.atleast_3d(np.array([[0],[0],[0],[1]])).T
    I0    = np.swapaxes(I0,1,2)
    mat_0 = np.repeat(I0,num_mic, axis = 0)
