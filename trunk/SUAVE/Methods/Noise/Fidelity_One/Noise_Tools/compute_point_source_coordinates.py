...
## @ingroupMethods-Noise-Fidelity_One-Noise_Tools 
def compute_point_source_coordinates(conditions,propellers,mls):  
    ...
    num_mic         = len(mls[0,:,0])
    num_prop        = len(propellers)
    
    I                        = np.atleast_3d(np.eye(4)).T
    rotation_2               = np.zeros((num_cpt,num_mic,num_prop,4,4))
    rotation_2[:,:,:,0:3,0:3]= conditions.frames.body.transform_to_inertial
