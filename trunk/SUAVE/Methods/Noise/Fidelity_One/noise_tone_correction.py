...
def noise_tone_correction(SPL):
    ...
    for i in range (1,23):
        if step3[i]!=0 and i<23:
            step4[i]=(SPL[i-1]+SPL[i+1])/2
        if step3[i]!=0 and i==23:
            step4[i]=SPL[i-1]+slope[i-1]
        if step3[i]==0:
            step4[i]=SPL[i]
    ...
    for i in range (17,22):
        step8_aux[i]=SPL[i]-step7[i]
        if step8_aux[i]>=1.5 and SPL[i]>0 and SPL[i+1]>0 and SPL[i-1]>0:
            step8[i]=step8_aux[i]
        else:
            step8[i]=0.
    
    step8_aux[23]=SPL[23]-step7[23]
    if step8_aux[23]>=1.5 and SPL[23]>0 and SPL[22]>0:
    ...
    #------------------------------------------------------------
    tone_correction_max=np.max(tone_correction)
