...
#Secondary jet source location
        ...
        XJ[i]=(zk*Diameter_mixed)*(2+1.6*np.arctan((4.5*theta_s[i]/np.pi)-2.25))*(1+0.5/np.sqrt(Str_s[i]))*np.sqrt(1+(0.7*Velocity_secondary/sound_ambient))*(Velocity_secondary/(Velocity_secondary-Velocity_aircraft))
