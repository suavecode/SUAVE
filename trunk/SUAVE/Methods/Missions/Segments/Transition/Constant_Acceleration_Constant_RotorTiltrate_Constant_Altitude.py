...
# Constant_Acceleration_Constant_RotorTiltrate_Constant_Altitude.py
# 
...
def initialize_conditions(segment):
    ...
    T0  = segment.tilt_initial
    Tf  = segment.tilt_final     
    ...
    # check for initial rotor tilt angle
    if T0 is None:
        T0  =  segment.state.initials.conditions.propulsion.y_axis_rotation
        segment.tilt_initial = T0    
    ...
    # set the rotor tilt angle
    if T0>Tf:
        # Gradually transition to forward flight
        tilt_angle = T0 + (time**2)*(Tf-T0)/((t_final-t_initial)**2)
    else:
        # Gradually transition towards hover
        tilt_angle = np.flip(Tf + (time**2)*(T0-Tf)/((t_final-t_initial)**2))
    
    segment.state.conditions.propulsion.propeller_y_axis_rotation = tilt_angle    
