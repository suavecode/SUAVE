...
from SUAVE.Optimization import Nexus as Nexus 
...
def differential_evolution(func, bounds, args=(), strategy='best1bin',
                           ...
                           workers=1, constraints=(), de_problem = ()):
    """Finds the global minimum of a multivariate function using genetic algorithm 
    method. Once criteria is met, there is the option to do polishing through 
    gradient descent (SLSQP)
    
    Source:
    Modified from Scipy Source Code 
    https://github.com/scipy/scipy/blob/v1.5.0/scipy/optimize/_differentialevolution.py#L22-L310 
    ...
    with DifferentialEvolutionSolver(func, bounds, args=args,
                                     ...
                                     de_problem=de_problem) as solver:
        ret = solver.solve()
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, args=(),
        ...
        self.problem = de_problem 
        de_cons =  de_problem.optimization_problem.constraints 
        
        self.polish_constraints = Nexus.diff_evo_constraints(de_problem)
        
        if hasattr(constraints, '__len__'):
            # sequence of constraints, this will also deal with default
            # keyword parameter
            for c in range(0,len(constraints)):
                self._wrapped_constraints.append(_ConstraintWrapper(de_problem ,de_cons[[c]], x0))
        else:
    ...
    def solve(self):
        ...
        if self.polish: 
            ...
            result = sp.optimize.fmin_slsqp(self.func, DE_result.x ,f_eqcons=self.problem.equality_constraint,\
                                            f_ieqcons= self.problem.inequality_constraint,bounds=self.limits.T,\
                                                     iter=200, epsilon = 1.4901161193847656e-08 ,full_output=1, acc  = 1e-6)            
            self._nfev += result[2]
            ...
            # within the bounds.
            if (result[1] < DE_result.fun and
                    result[3] == 0 and
                    np.all(result[0] <= self.limits[1]) and
                    np.all(self.limits[0] <= result[0])):
                DE_result.fun = result[1].astype('Float64')
                DE_result.x = result[0]
                # to keep internal state consistent
                self.population_energies[0] = result[1]
                self.population[0] = self._unscale_parameters(result[0])
...
class _ConstraintWrapper(object):
    ...
    bounds : 2-tuple
        ...
        the constraints.
    """
    def __init__(self, problem , constraint, x0):
        
        problem.evaluate(x0)
        aliases        = problem.optimization_problem.aliases        
        bound          = help_fun.scale_const_bnds(constraint)
        
        def fun(x):
            problem.objective(x)
            constraint_val = help_fun.get_values(problem ,constraint,aliases)  
        ...
        lb = np.asarray(low_bnd, dtype=float)
        ub = np.asarray(up_bnd, dtype=float)
    ...
    def violation(self, x):
        ...
        e_v   = self.fun(np.asarray(x))
        ev    = e_v.astype('Float64')
        
        excess_lb = np.maximum(self.bounds[0] - ev[0], 0)
        excess_ub = np.maximum(ev[0] - self.bounds[1], 0)
