""" PASS_Aero.py: Aerodyanmic properties in PASS style """
...
class PASS_Aero(Aerodynamics):

    """ PASS_Aero: Aerodyanmic properties in PASS style """
    ...
    ### ----------------------------------------------------------------------
    ### Compute Cl for a wing using lifting line theory
    ### ----------------------------------------------------------------------
    #def clift_wing():

    #def aircraft_lift(vehicle,state):
    ...
    def cdp_misc(self,sweep_w, sweep_h, sweep_v, d_engexit,Sref,Mc,roc,muc ,Tc,S_affected_w,S_affected_h,S_affected_v):
        pi=22/7
        ...
        #------------Nacelle base drag--------------------------------
        cd_nacelle_base=0.5/12*pi*d_engexit*0.2/Sref
        ...
        cd_misc = cd_gaps+cd_nacelle_base+cd_upsweep  #+cd_trim
        #print(cd_misc)
        return cd_misc
    ...
    def cdp(self,l_fus, d_fus,l_nose,l_tail,mac_w,t_c_w,sweep_w, S_exposed_w, mac_h,t_c_h,sweep_h, S_exposed_h,mac_v,t_c_v,sweep_v, S_exposed_v,d_engexit,Sref,Mc,roc,muc ,Tc,S_affected_w,S_affected_h,S_affected_v):
        ...
        cdp_miscel=self.cdp_misc(sweep_w, sweep_h, sweep_v, d_engexit,Sref,Mc,roc,muc ,Tc,S_affected_w,S_affected_h,S_affected_v)
    ...
    #def cdc(self,Minf):
        ...
        tc=t_c_w/numpy.cos(sweep_w)
        cl=Cl/(numpy.cos(sweep_w))**2
    ...
    #obtained from aero itself wait for lift model 
        ...
        [cd_p,cdp_w,cdp_v,cdp_h]=self.cdp(l_fus, d_fus,l_nose,l_tail,mac_w,t_c_w,sweep_w, S_exposed_w, mac_h,t_c_h,sweep_h, S_exposed_h,mac_v,t_c_v,sweep_v, S_exposed_v,d_engexit,Sref,Mc,roc,muc ,Tc,S_affected_w,S_affected_h,S_affected_v)
    ...
    def initialize(self,vehicle):
        ...
        #alpha_range=21
        #Cl_a=numpy.empty(alpha_range)
        #aoa_range=numpy.empty(alpha_range)
        #for i in range(0,10):  

            #aoa_range[i]=i*numpy.pi/180
            ##print aoa_range[i]
            #Cl_a[i]=self.aircraft_lift(vehicle,aoa_range[i])
        #print 'aoa_range',aoa_range
        #self.f_Cl = interp1d(aoa_range, Cl_a,bounds_error=False,fill_value=0.)
        ...
        self.CL0 = Cl_a[0]
        self.dCLdalpha = (Cl_a[2]-Cl_a[1])/((aoa_range[2]-aoa_range[1])*numpy.pi/180)


        #print 'PAss aero CL_a' , Cl_a
        #print 'Pass aero dCLdalpha',self.dCLdalpha 
        #print aoaa


        #print 'PassAero Cltest', Cl_test
