...
from SUAVE.Methods.Aerodynamics.AVL_Surrogate.Drag import compute_aircraft_drag
...
class AVL_Surrogate(Aerodynamics_Surrogate):
    ...
    def initialize(self,vehicle):
        ...
        configuration.aircraft_span_efficiency_factor = vehicle.wings[0].span_efficiency
        ...
        konditions.aerodynamics.drag_breakdown.induced = Result(
                total             = 0.0 ,
                efficiency_factor = configuration.aircraft_span_efficiency_factor  ,
                aspect_ratio      = geometry.wings[0].aspect_ratio ,
            )         
        ...
        for i in xrange(n_conditions):
            ...
            CDi[i] = calculate_induced_drag(konditions, configuration, geometry)

        # store table
        conditions_table.lift_coefficient         = CL
        conditions_table.induced_drag_coefficient = CDi
    ...
    def build_surrogate(self):
        ...
        CDi_data = conditions_table.induced_drag_coefficient

        # pack and reshape for surrogate
        X_data = np.array([AoA_data]).T
        X_data = np.reshape(X_data,-1)
        
        # assign models
        Interpolation_CL  = np.poly1d(np.polyfit(X_data, CL_data ,1))
        Interpolation_CDi = np.poly1d(np.polyfit(X_data, CDi_data, 2))

        #Interpolation = Fidelity_Zero.Interpolation
        self.models.lift_coefficient = Interpolation_CL
        self.models.induced_drag_coefficient = Interpolation_CDi
    ...
    def __call__(self,conditions):
        ...
        CL = SUAVE.Methods.Aerodynamics.AVL_Surrogate.Lift.compute_aircraft_lift(conditions,configuration,geometry)
...
def calculate_lift_vortex_lattice(conditions,configuration,geometry):
    ...
    conditions.aerodynamics.lift_coefficient = total_lift_coeff

    return total_lift_coeff



def calculate_induced_drag(conditions,configuration,geometry):
    """ calculate induced drag of a wing
    """
    
    # import
    from SUAVE.Methods.Aerodynamics.Fidelity_Zero.Drag.induced_drag_aircraft import induced_drag_aircraft
    
    cdi = induced_drag_aircraft(conditions, configuration, geometry)
