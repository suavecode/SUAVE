...
class AVL(Stability):
	...
	Cn_beta = np.zeros([len(AoA)*len(mach),1]) 
		...
		Cn_beta[count*len(mach):(count+1)*len(mach),0] = results.aerodynamics.cn_beta[:,0]
            ...
            CD         = data_array[:,3:4]
            CM         = data_array[:,4:5]
            Cm_alpha   = data_array[:,5:6]
	    Cn_beta    = data_array[:,6:7]
        # Save the data
        np.savetxt(geometry.tag+'_data.txt',np.hstack([xy,CL,CD,CM,Cm_alpha, Cn_beta]),fmt='%10.8f',header='AoA Mach CL CD')

        # Store training data
        training.coefficients = np.hstack([CL,CD,CM,Cm_alpha, Cn_beta])
	...
	Cm_alpha_data = training.coefficients[:,3]
	Cn_beta_data = training.coefficients[:,4]
	...
	regr_cn_beta = gaussian_process.GaussianProcess()
        ...
        cm_alpha_surrogate = regr_cm.fit(xy, Cm_alpha_data) 
	cn_beta_surrogate = regr_cm.fit(xy, Cn_beta_data)
