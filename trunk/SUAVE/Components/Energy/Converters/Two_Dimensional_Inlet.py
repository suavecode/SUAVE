...
class Two_Dimensional_Inlet(Energy_Component):
    ...
    def compute(self, conditions):
        ...
        theta = self.angles.ramp_angle*np.pi/180. # incoming angle for the shock in degrees
        ...
        beta[i_sup] = Oblique_Shock.theta_beta_mach(M0[i_sup],gamma[i_sup],theta)
        MC[i_sup], Pr_c[i_sup], Tr_c[i_sup], Ptr_c[i_sup] = Oblique_Shock.oblique_shock_relations(M0[i_sup],gamma[i_sup],theta,beta[i_sup])
    ...
    #     theta = self.angles.ramp_angle*np.pi/180.
    ...
    #         Ps_ov_Pinf = Oblique_Shock.get_invisc_press_recov(theta, M1)
