...
class Propeller(Energy_Component):
    ...
    def spin(self,conditions):
        ...
        lambda_tot   = (np.atleast_2d(V_inf[:,0]).T + ua)/(omega*R)
        lambda_i     = (np.atleast_2d(ua)/(omega*R))
        
        mu_prop      = (np.atleast_2d(V_inf[:,2]).T) /(omega*R)  
        alpha_disc   = np.arctan(np.atleast_2d(V_inf[:,0]).T/V_inf[:,2])
        lambda_c     = (np.atleast_2d(V_inf[:,0]).T)/(omega*R)
        ...
        if test_BET:             
            ...
            lambda_c        = np.tile(np.atleast_3d(lambda_c),(1,N,N))
            lambda_i        = np.tile(np.atleast_3d(lambda_i),(1,N,N))
            lambda_tot      = np.tile(np.atleast_3d(lambda_tot),(1,N,N))
            ...
            lambda_i = lambda_i*(1 + kx*r_2d*np.cos(psi_2d))     
            lambda_tot   = lambda_c + lambda_i
            ...
            up = lambda_tot*omega_2d*R  + r_2d*beta_dot + mu_prop*omega_2d*R*beta*np.cos(psi_2d)  
    ...
    def spin_new(self,conditions): #Unpack            
        B                 = self.number_blades
        R                 = self.tip_radius
        Rh                = self.hub_radius
        local_blade_pitch = self.twist_distribution
        ...
        if Vh != None:     
            for i in range(len(V)): 
                ...
                elif Vv[i][0]  == 0 and  Vv[i][2] != 0:
                    # climbing 
                    if V_Vh > 0: 
                        ua[i] = - V_inf[i][0]/2  - np.sqrt((V_inf[i][0]/2)**2 + Vh**2) # eqn 4.8 (page 94 Leishman)
                    # slow descent /vortex ring state    
                    elif -2 <= V_Vh and V_Vh <= 0:                
                        ua[i] = kappa*V_inf[i][0]*(0.373*(V_inf[i]/Vh)**2  - 1.991)  # eqn 4.13 (page 101 Leishman )
                    # windmilling     
                    else: 
                        print("rotor is in the windmill break state!")
                        ua[i] = Vh*(-(-V_inf[i][0]/(2*Vh)) - np.sqrt((-V_inf[i][0]/(2*Vh))**2 + 1))
