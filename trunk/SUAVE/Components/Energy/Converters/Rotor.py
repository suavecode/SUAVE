...
class Rotor(Energy_Component):
    ...
    def spin(self,conditions):
        ...
        mu_rot      = (np.atleast_2d(V_inf[:,2]).T) /(omega*R)           # rotor advance ratio  (page 30 Leishman)
        ...
        if any(mu_lambda[:,0] < 10.0) or use_BET:  
            ...
            # 2-D inflow ratio 
            mu_2d     = np.tile(np.atleast_2d(mu_rot),(1,Nr))
