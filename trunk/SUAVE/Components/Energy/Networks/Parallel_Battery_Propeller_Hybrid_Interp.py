...
class Series_Battery_Propeller_Hybrid_Interp(Propulsor):
    ...
    def evaluate_thrust(self,state):
        ...
        # Figure out if the power is more than the motor can handle, or the battery is dead
        Pmotor = P*1.0/n_motor
        pmotor_max = self.motor.rated_power
        Pmotor[Pmotor>pmotor_max] = pmotor_max
        
        Pmotor[battery.current_energy<0.] = 0.
        torque_motor = Pmotor/omega
        
        # Now figure out if the internal combustion engine power
        P_IC = P-Pmotor*n_motor
        P_IC[P_IC>engine.sea_level_power] = engine.sea_level_power
        
        # Figure out if we need magic power
        magic_power = P-Pmotor*n_motor-P_IC
        magic_power[magic_power<0.] = 0.
        
        # Run the motor
        motor.inputs.omega  = omega
        motor.inputs.torque = torque_motor
        ...
        power_to_gen  = engine.outputs.power-P_IC
        torque_to_gen = power_to_gen/engine.speed
        
        generator.inputs.torque = torque_to_gen
