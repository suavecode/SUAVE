...
class Battery_Propeller(Propulsor):
    def __defaults__(self): 
        ...
        self.thrust_motor      = None
        self.lift_prop         = None
        self.thrust_prop       = None
        self.lift_esc          = None
        self.thrust_esc        = None
        self.avionics          = None
        self.payload           = None
        self.battery           = None
        self.nacelle_diameter  = None
        self.engine_length     = None
        self.num_lift_rotors   = None
        self.num_thrust_rotors = None
        self.voltage           = None
        self.tag               = 'network'
    ...
    def evaluate_thrust(self,state):
        ...
        lift_motor      = self.motor
        thrust_motor    = self.thrust_motor
        lift_prop       = self.lift_prop
        thrust_prop     = self.thrust_prop
        lift_esc        = self.lift_esc
        thrust_esc      = self.thrust_esc
        ...
        lift_esc.inputs.voltagein = state.unknowns.battery_voltage_under_load        
        thrust_esc.inputs.voltagein = state.unknowns.battery_voltage_under_load
        # Step 2
        lift_esc.voltageout(conditions)
        thrust_esc.voltageout(conditions)
        # link
        lift_motor.inputs.voltage = lift_esc.outputs.voltageout
        thrust_motor.inputs.voltage = thrust_esc.outputs.voltageout 
        # step 3
        lift_motor.omega(conditions)
        thrust_motor.omega(conditions)
        # link
        lift_prop.inputs.omega =  lift_motor.outputs.omega
        lift_prop.thrust_angle = 90
        thrust_prop.inputs.omega =  lift_motor.outputs.omega
        thrust_prop.thrust_angle = 0        
        # step 4
        lift_F, lift_Q, lift_P, lift_Cp = lift_prop.spin(conditions)
        
        
        # MODIFICATIONS FROM BATTERY PROPELLER STOP HERE - CURRENTLY NON-FUNCTIONAL        
