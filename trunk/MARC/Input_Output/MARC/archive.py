...
def archive(data,filename):
    """Converts a MARC data structure to a JSON file for storage. 
...
def build_dict_base(base):
    """Builds a dictionary based on a MARC data structure. This is initial case.
...
## @ingroup Input_Output-MARC
def build_dict_r(v):
    """Builds a dictionary based on a MARC data structure. This the recursive step.
    ...
    if (tv == np.ndarray) or (tv == np.float64):
        ...
        # Assume other data types are MARC data types and check
        try:
            keys = v.keys()
        except:
            if callable(tv):
                ...
                raise TypeError('Unexpected data type in MARC data structure')
