...
from MARC.Components.Airfoils.Airfoil import Airfoil
from MARC.Methods.Geometry.Two_Dimensional.Planform import wing_planform, wing_segmented_planform
import numpy as np
import string
try:
    import vsp as vsp
...
def read_vsp_wing(wing_id, units_type='SI', write_airfoil_file=True, use_scaling=True):
    """This reads an OpenVSP wing vehicle geometry and writes it into a MARC wing format.
    ...
    Outputs:
    Writes MARC wing object, with these geometries, from VSP:
    ...
    if single_seg == False:
        ...
        # Convert VSP XSecs to MARC segments. (Wing segments are defined by outboard sections in VSP, but inboard sections in MARC.)
        for i in range(1, segment_num+1):
            # XSec airfoil
            jj = i-1  # Airfoil index i-1 because VSP airfoils and sections are one index off relative to MARC.
            ...
            if i < segment_num:      # This excludes the tip xsec, but we need a segment in MARC to store airfoil.
    ...
    for cs_idx in range(num_cs):
        ...
        if LE_flags[cs_idx] == 1.0:
            CS = MARC.Components.Wings.Control_Surfaces.Slat()
        else:
            if wing.vertical == True:
                CS = MARC.Components.Wings.Control_Surfaces.Rudder()
            else:
                if aileron_present:
                    CS = MARC.Components.Wings.Control_Surfaces.Aileron()
                else:
                    CS = MARC.Components.Wings.Control_Surfaces.Flap()
        CS.tag                 = tags[cs_idx]
        ...
        if CS.span_fraction_start > 1 or CS.span_fraction_end < 0:
            raise AssertionError("MARC import of VSP files does not allow control surfaces defined for the wing caps.")
...
def write_vsp_wing(vehicle,wing, area_tags, fuel_tank_set_ind, OML_set_ind):
    ...
    if wing.vertical == True:
        ...
        dihedral = -dihedral # check for vertical tail, direction reverses from MARC/AVL
    ...
    for i_segs in range(1,n_segments+1):
        ...
        # OpenVSP's rotation on vertical segmented wings is opposite of MARC's
...
def write_vsp_control_surface(wing,wing_id,ctrl_surf):
    ...
    for p_idx in range(len(param_names)):
        if 'LE_Flag' == vsp.GetParmName(param_names[p_idx]):
            if type(ctrl_surf) == MARC.Components.Wings.Control_Surfaces.Slat:
...
def write_wing_conformal_fuel_tank(vehicle,wing, wing_id,fuel_tank,fuel_tank_set_ind):
    ...
    if n_segments>0:
        span_trim_max = get_vsp_trim_from_MARC_trim(seg_span_percents,
                                                     vsp_segment_breaks,
                                                             span_trim_max)
        span_trim_min = get_vsp_trim_from_MARC_trim(seg_span_percents,
                                                     vsp_segment_breaks,
                                                             span_trim_min)
    else:
...
## @ingroup Input_Output-OpenVSP
def get_vsp_trim_from_MARC_trim(seg_span_percents,vsp_segment_breaks,trim):
    """Compute OpenVSP span trim coordinates based on MARC coordinates
    ...
    trim                [-] range of 0 to 1 (MARC value)
