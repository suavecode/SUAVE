...
def read_vsp_fuselage(fuselage_id,fux_idx,sym_flag, units_type='SI', fineness=True, use_scaling=True):
    """This reads an OpenVSP fuselage geometry and writes it to a MARC fuselage format.
    ...
    Writes MARC fuselage, with these geometries:           (all defaults are SI, but user may specify Imperial)
...
def write_vsp_fuselage(fuselage,area_tags, main_wing, fuel_tank_set_ind, OML_set_ind):
    ...
    if num_segs==0: # MARC default fuselage shaping
...
def compute_fuselage_fineness(fuselage, x_locs, eff_diams, eff_diam_gradients_fwd):
    ...
    Outputs:
    Writes fineness values to MARC fuselage, returns fuselage.
...
def get_fuselage_height(fuselage, location):
    ...
    Inputs:
    0. Pre-loaded VSP vehicle in memory, via vsp_read.
    1. MARC fuselage [object], containing fuselage.vsp_data.xsec_num in its data structure.
