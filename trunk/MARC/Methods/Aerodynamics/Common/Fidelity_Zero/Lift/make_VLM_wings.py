...
from MARC.Methods.Geometry.Two_Dimensional.Planform import populate_control_sections
from MARC.Methods.Flight_Dynamics.Static_Stability.Approximations.Supporting_Functions import convert_sweep_segments
...
def make_VLM_wings(geometry, settings):
    ...
    for wing in wings:
        ...
        seg_breaks  = MARC.Core.ContainerOrdered()
        LE_breaks   = MARC.Core.ContainerOrdered()
        TE_breaks   = MARC.Core.ContainerOrdered()
        n_segments  = len(wing.Segments.keys())
        ...
        span_breaks = MARC.Core.ContainerOrdered()        
        n_LE        = len(LE_breaks)
        n_TE        = len(TE_breaks)
...
def copy_large_container(large_container, type_str):
    ...
    """    
    container = MARC.Core.Container()  if type_str != "Segments" else MARC.Core.ContainerOrdered()
...
def make_cs_wing_from_cs(cs, seg_a, seg_b, wing, cs_ID):
    ...
    cs_wing                       = copy_data_from_paths(MARC.Components.Wings.Wing(), get_paths("wings"))
...
def reprocess_span_breaks(span_breaks):
    ...
    span_breaks
    """     
    sbs = MARC.Core.ContainerOrdered()
