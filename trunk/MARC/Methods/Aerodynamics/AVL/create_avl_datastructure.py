...
from MARC.Components.Wings.Control_Surfaces                             import Aileron , Elevator , Slat , Flap , Rudder 
from MARC.Methods.Aerodynamics.AVL.write_avl_airfoil_file               import write_avl_airfoil_file   
## @ingroup Methods-Aerodynamics-AVL

def translate_avl_wing(marc_wing):
    ...
    Inputs:
        marc_wing.tag                                                          [-]
        marc_wing.symmetric                                                    [boolean]
        marc_wing.verical                                                      [boolean]
        marc_wing - passed into the populate_wing_sections function            [data stucture]
    ...
    w                 = Wing()
    w.tag             = marc_wing.tag
    w.symmetric       = marc_wing.symmetric
    w.vertical        = marc_wing.vertical
    w                 = populate_wing_sections(w,marc_wing)

    return w

def translate_avl_body(marc_body):
    ...
    Inputs:
        body.tag                                                       [-]
        marc_wing.lengths.total                                       [meters]    
        marc_body.lengths.nose                                        [meters]
        marc_body.lengths.tail                                        [meters]
        marc_wing.verical                                             [meters]
        marc_body.width                                               [meters]
        marc_body.heights.maximum                                     [meters]
        marc_wing - passed into the populate_body_sections function   [data stucture]
    ...
    b.tag             = marc_body.tag
    b.symmetric       = True
    b.lengths.total   = marc_body.lengths.total
    b.lengths.nose    = marc_body.lengths.nose
    b.lengths.tail    = marc_body.lengths.tail
    b.widths.maximum  = marc_body.width
    b.heights.maximum = marc_body.heights.maximum
    b                 = populate_body_sections(b,marc_body)

    return b

def populate_wing_sections(avl_wing,marc_wing): 
    ...
    Inputs:
        ...
        marc_wing.spans.projected                 [meters]
        marc_wing.origin                          [meters]
        marc_wing.dihedral                        [radians]
        marc_wing.Segments.sweeps.leading_edge    [radians]
        marc_wing.Segments.root_chord_percent     [-]
        marc_wing.Segments.percent_span_location  [-]
        marc_wing.Segments.sweeps.quarter_chord   [radians]
        marc_wing.Segment.twist                   [radians]
    ...
    semispan             = marc_wing.spans.projected*0.5 * (2 - symm)
    avl_wing.semispan    = semispan   
    root_chord           = marc_wing.chords.root
    segments             = marc_wing.Segments
    n_segments           = len(segments.keys()) 
    origin               = marc_wing.origin  
        
    if len(marc_wing.Segments.keys())>0: 
        for i_segs in range(n_segments):
            ...
            if (i_segs == n_segments-1):
                ...
                if getattr(marc_wing,'control_surfaces',False):   
                    ...
                    append_avl_wing_control_surfaces(marc_wing,avl_wing,semispan,root_chord_percent,tip_chord_percent,tip_percent_span,
        ...
        dihedral              = marc_wing.dihedral
        if marc_wing.sweeps.leading_edge  is not None: 
            sweep      = marc_wing.sweeps.leading_edge
        else:  
            sweep_quarter_chord = marc_wing.sweeps.quarter_chord 
            chord_fraction      = 0.25                          
            segment_root_chord  = marc_wing.chords.root
            segment_tip_chord   = marc_wing.chords.tip
        ...
        root_section.chord    = marc_wing.chords.root 
        root_section.twist    = marc_wing.twists.root/Units.degrees
        
        # append control surfaces
        if marc_wing.Airfoil:
            tip_airfoil  = marc_wing.Airfoil.airfoil.coordinate_file     
        else:
            tip_airfoil = None
        seg_tag            = 'section'   
        append_avl_wing_control_surfaces(marc_wing,avl_wing,semispan,1.0,marc_wing.taper,1.0,
                                         0.0,marc_wing.twists.root,marc_wing.twists.tip,tip_airfoil,seg_tag,dihedral,origin,sweep)  
        
        # define tip section
        tip_section           = Section()
        tip_section.tag       = 'tip_section'
        tip_section.chord     = marc_wing.chords.tip 
        tip_section.twist     = marc_wing.twists.tip/Units.degrees  
        ...
        # assign wing airfoil
        if marc_wing.Airfoil:
            root_section.airfoil_coord_file  = marc_wing.Airfoil.airfoil.coordinate_file          
            tip_section.airfoil_coord_file   = marc_wing.Airfoil.airfoil.coordinate_file     
...
def append_avl_wing_control_surfaces(marc_wing,avl_wing,semispan,root_chord_percent,tip_chord_percent,tip_percent_span,root_percent_span,root_twist,tip_twist,tip_airfoil,seg_tag,dihedral,origin,sweep):

    """ Converts control surfaces on a marc wing to sections in avl wing
    ...
    root_chord    = marc_wing.chords.root                    
    section_spans = []
    for cs in marc_wing.control_surfaces:     
    ...
    for section_count in range(num_sections):        
        ...
        for index  , ctrl_surf in enumerate(marc_wing.control_surfaces):
...
def populate_body_sections(avl_body,marc_body):
    ...
    vec2 = [1  ,1.57 , 3.2,  8]
    x = np.linspace(0,1,4)
    fuselage_nose_curvature =  np.interp(np.interp(marc_body.fineness.nose,vec2,x), x , vec1)
    fuselage_tail_curvature =  np.interp(np.interp(marc_body.fineness.tail,vec2,x), x , vec1) 
