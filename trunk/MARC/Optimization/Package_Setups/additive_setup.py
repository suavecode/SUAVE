...
from MARC.Methods.Utilities.latin_hypercube_sampling import latin_hypercube_sampling
...
class Additive_Solver():
    def __init__(self):
        ...
        # therefore are always available when running MARC
        ...
        for kk in range(max_iterations):
            ...
            if opt_type == 'basic': # Next point determined by surrogate optimum
                ...
                if self.local_optimizer == 'SNOPT':
                    opt_prob = pyOpt.Optimization('MARC',self.evaluate_corrected_model, \
                ...
                if self.global_optimizer == 'ALPSO':
                    opt_prob = pyOpt.Optimization('MARC',self.evaluate_expected_improvement, \
