...
def plot_mission(results):
    ...
    #   Battery Energy
    # ------------------------------------------------------------------
    #plt.figure("Solar Outputs")  
    fig = plt.figure("Solar Outputs")
    ...
    axes.grid(True)   
    #plt.savefig('Battery_Energy.pdf')
    ...
    axes.grid(True)   
    #plt.savefig('Solar_Flux.pdf')
    
    # ------------------------------------------------------------------    
    #   Battery Draw
    # ------------------------------------------------------------------
    #plt.figure("Available Power to Charge Battery")
    #axes = plt.gca()    
    axes = fig.add_subplot(3,1,2)
    for i in range(len(results.segments)):     
        time     = results.segments[i].conditions.frames.inertial.time[:,0] / Units.min
    ...
    #for i in range(len(results.segments)):     
        #time     = results.segments[i].conditions.frames.inertial.time[:,0] / Units.min
        #etap = results.segments[i].conditions.propulsion.etam[:,0] 
        #axes.plot(time, etap, 'bo-')
    #axes.set_xlabel('Time (mins)')
    #axes.set_ylabel('Etap')
    #axes.get_yaxis().get_major_formatter().set_scientific(False)
