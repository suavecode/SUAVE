...
#http://www.pyopt.org/examples/examples.parallel_gradient.html
...
def main():
    ...
    opt_problem = pyOpt.Optimization('Max Range',mywrap)
    ...
    outputs = opt(opt_problem,sens_type='FD',sens_mode='pgc')
    
    if myrank == 0:
    
        vehicle.Mass_Props.m_empty = outputs[1][0]*10000.0
        vehicle.Mass_Props.m_takeoff = outputs[1][1]*10000.0    
...
def opt_func(inputs,vehicle,mission):
    ...
    constraints = [(m_fuel_cell-m_fuel_cell_req)/10000.0,(m_takeoff-m_fuel-m_empty)/10000.0]
    ...
    return output/10000.0 , constraints , fail
