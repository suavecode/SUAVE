...
def vehicle_setup():
    ...
    net.nacelle_diameter              = 0.5*Units.feet #np.array([1.66, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5,
                                                  #0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1.66]) * Units.feet
    net.engine_length                 = 1.0*Units.feet #np.array([3., 1., 1., 1., 1., 1., 1.,
    ...
    tip_prop.number_of_blades       = 3
    tip_prop.freestream_velocity    = 135.*Units['mph']    
    tip_prop.angular_velocity       = 1500.  * Units.rpm  
    tip_prop.tip_radius             = 1.523/2
    tip_prop.hub_radius             = 0.15*tip_prop.tip_radius
    tip_prop.design_Cl              = 0.75
    tip_prop.design_altitude        = 8000. * Units.feet
    tip_prop.design_tip_mach        = 0.4
    tip_prop.design_thrust          = 400.0
    ...
    highlift_prop.freestream_velocity    = 80.*Units['mph']    
    highlift_prop.angular_velocity       = 4800.  * Units.rpm  
    highlift_prop.tip_radius             = 0.58/2
    highlift_prop.hub_radius             = 0.15*tip_prop.tip_radius
    highlift_prop.design_Cl              = 0.75
    highlift_prop.design_altitude        = 5000. * Units.feet
    highlift_prop.design_tip_mach        = 0.4
    highlift_prop.design_thrust          = 100.0
    ...
    net.number_of_propeller_engines   = len(net.propellers)
    # --------------------------------------------------------------------
    ...
    for i in range(len(props)):
        ...
        m.mass_properties.mass    = 10. * Units.kg        
...
def configs_setup(vehicle):
    ...
    # ------------------------------------------------------------------    
    # Takeoff config: Uses tip props, augmented with distributed props
    # ------------------------------------------------------------------
    takeoff_vehicle = deepcopy(vehicle)

    takeoff_config = SUAVE.Components.Configs.Config(takeoff_vehicle)
    takeoff_config.tag = 'takeoff'
    takeoff_config.networks.battery_propeller.pitch_command = 0.
    configs.append(takeoff_config)       

    # ------------------------------------------------------------------    
    # Cruise config: Only uses tip-props, remove distributed props
    # ------------------------------------------------------------------
    cruise_vehicle = deepcopy(vehicle)
    all_props      = vehicle.networks.battery_propeller.propellers
    prop_removal   = "high_lift_propeller"
    
    # turn off distributed propellers
    for i in range(len(all_props)):
        motor_key = list(vehicle.networks.battery_propeller.propeller_motors.keys())[i]
        prop_key  = list(vehicle.networks.battery_propeller.propellers.keys())[i]
        if prop_removal in prop_key:
            # remove this prop and motor from cruise analysis
            del cruise_vehicle.networks.battery_propeller.propellers[prop_key]
            del cruise_vehicle.networks.battery_propeller.propeller_motors[motor_key]
    cruise_vehicle.networks.battery_propeller.number_of_propeller_engines = len(cruise_vehicle.networks.battery_propeller.propellers)
    
    cruise_config = SUAVE.Components.Configs.Config(cruise_vehicle)
    cruise_config.tag = 'cruise'
    cruise_config.networks.battery_propeller.pitch_command = 0.
    configs.append(cruise_config)      
    
    
    # ------------------------------------------------------------------    
    # Climb config: Only uses tip props, distributed props turned off
    # ------------------------------------------------------------------
    climb_vehicle = deepcopy(cruise_vehicle)

    climb_config = SUAVE.Components.Configs.Config(climb_vehicle)
    climb_config.tag = 'climb'
    climb_config.networks.battery_propeller.pitch_command = 0.
    configs.append(climb_config)      
    
    
    
    ## ------------------------------------------------------------------    
    ## Takeoff config: Only uses distributed props, remove tip props
    ## ------------------------------------------------------------------
    #takeoff_vehicle = deepcopy(vehicle)
    #all_props      = vehicle.networks.battery_propeller.propellers
    #prop_removal   = "cruise_propeller"
    ...
    #for i in range(len(all_props)):
        ...
        #if prop_removal in prop_key:
            ## remove this prop and motor from cruise analysis
            #del takeoff_vehicle.networks.battery_propeller.propellers[prop_key]
            #del takeoff_vehicle.networks.battery_propeller.propeller_motors[motor_key]
    #takeoff_vehicle.networks.battery_propeller.number_of_propeller_engines = len(takeoff_vehicle.networks.battery_propeller.propellers)
    
    #climb_config = SUAVE.Components.Configs.Config(takeoff_vehicle)
    #climb_config.tag = 'takeoff'
    #climb_config.networks.battery_propeller.pitch_command = 0.
    #configs.append(climb_config)        
