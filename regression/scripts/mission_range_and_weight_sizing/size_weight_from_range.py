...
from SUAVE.Core import Units
from SUAVE.Methods.Performance.size_weights_given_mission_range import size_weights_given_mission_range
import sys
...
def main():
    ...
    cruise_segment_tag = mission.segments.cruise.tag
    mission_payload    = 12000. 
    target_range       = 2500 * Units.miles 
    
    distance,fuel,tow = size_weights_given_mission_range(vehicle,mission,cruise_segment_tag,mission_payload,target_range)
    
    truth_distance = 4024627.94789228
    distance_error = np.max(np.abs(distance[0]-truth_distance)) 
    print('Range Error = %.4e' % distance_error)
   
    truth_fuel = 9581.84513489
    fuel_error = np.max(np.abs(fuel[0]-truth_fuel)) 
    print('Fuel Weight Error= %.4e' % fuel_error)
    
    truth_tow =  51226.52397692
    tow_error = np.max(np.abs(tow[0]-truth_tow)) 
    print('Take off Weight Error= %.4e' % tow_error)    
