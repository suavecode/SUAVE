...
from Concorde_Inlet import vehicle_setup, configs_setup
...
def main():
    ...
    #old_results = load_results('results_mission_concorde_original.res')

    # plt the old results
    #plot_mission(results,'.b-',legend_label='with inlet')
    #plot_mission(old_results,'.k-', legend_label='original')
    ...
    results = load_results('results_mission_concorde_inlet_dragAE_095.res') 
    plot_mission(results,'.b-')
    
    results = load_results('results_mission_concorde_inlet_dragAE_105.res') 
    plot_mission(results,'.c-')
    
    results = load_results('results_mission_concorde_inlet_drag.res') 
    plot_mission(results,'.k-')    
    
    #results = load_results('results_mission_concorde_inlet_dragtheta15.res') 
    #plot_mission(results,'.b-')   
...
def plot_mission(results,line_style='bo-',legend_label=''):
    ...
    #for segment in list(results.segments.values()):
        ...
        ##residual = segment.state.residuals.network[:,0]
        ...
        #axes.plot( time , residual , line_style )
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('Res')
        #axes.grid(True)          
    ...
    #for segment in list(results.segments.values()):
        ...
        #Drag   = -segment.conditions.frames.wind.drag_force_vector[:,0]
        #Thrust = segment.conditions.frames.body.thrust_force_vector[:,0]
        #inlet_drag = segment.conditions.freestream.inlet_drag*0.1
        ...
        #axes.plot( time , Thrust/inlet_drag   , line_style )
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('Thrust/Inlet Drag (N)')
        #axes.grid(True)
    ...
    for segment in list(results.segments.values()):
        ...
        area          = segment.conditions.freestream.area_initial_streamtube[:,0]/(3*1.1/2.**2*np.pi* Units['meter**2'])
        
        #mach          = segment.conditions.freestream.mach[:,0]
        #Pt_out        = segment.conditions.freestream.Pt_out[:,0]
        #T_out         = segment.conditions.freestream.Tout[:,0]
        ...
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('Inlet exit mach')
        #axes.grid(True)
        
        #axes = fig.add_subplot(3,1,2)
        #axes.plot( time , Pt_out , line_style )
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('Inlet exit stagnation pressure (Pa)')
        #axes.grid(True)
        
        #axes = fig.add_subplot(3,1,3)
        #axes.plot( time , T_out , line_style )
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('Inlet exit temperature (K)')
        #axes.grid(True)        
    ...
    #for i in range(len(results.segments)):
        ...
        #inlet_drag = -results.segments[i].conditions.freestream.inlet_drag[:,0]
        #drag = inlet_drag/Thrust.transpose()       
        #axes.plot( time , drag , line_style )
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('Inlet Drag/Thrust')    
        #axes.grid(True)        
    ...
    #for segment in list(results.segments.values()): 
        #time   = segment.conditions.frames.inertial.time[:,0] / Units.min
        #area          = segment.conditions.freestream.area_initial_streamtube[:,0]/(3*1.1/2.**2*np.pi* Units['meter**2'])
        #axes.plot( time , area , line_style )
        #axes.set_xlabel('Time (min)')
        #axes.set_ylabel('A0/AC')
        #axes.grid(True)        
        
    
    
    fig = plt.figure("A0/AC")
    ...
    #axes.set_ylabel('L/D')
    #axes.grid(True)   
    #axes.legend(['original','with inlet'])
...
def load_results(filename='results_mission_concorde_original.res'):
    return SUAVE.Input_Output.SUAVE.load(filename)

def save_results(results):
    SUAVE.Input_Output.SUAVE.archive(results,'results_mission_concorde_inlet_flow.res')
    return    
