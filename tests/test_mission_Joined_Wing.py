""" test_mission_Joined_Wing.py: evaluate a mission with a Boeing 737-800 derivative Joined_Wing Concept """
...
def main():
    ...
    Joined_Wing = SUAVE.Vehicle()
    Joined_Wing.tag = "Joined Wing"
    

#----------------------mass property definition------------------------------------------    
    
    Joined_Wing.Mass_Props.m_full = 78799.8#67746.4             # kg
    Joined_Wing.Mass_Props.m_empty = 75089.9#62746.4             # kg (Actually Zero-Fuel Weight)
    Joined_Wing.Mass_Props.m_takeoff = 78799.8 #67746.4         # kg
    
    Joined_Wing.Mass_Props.m_flight_min = 0.1*(Joined_Wing.Mass_Props.m_full -Joined_Wing.Mass_Props.m_empty)+Joined_Wing.Mass_Props.m_empty  #63246.4        # kg (10 percent fuel as reserve)

#-----------------------basic parameters---------------------------------------------------

    Joined_Wing.delta = 25.0                           # deg
    Joined_Wing.S = 124.862 # (value used in CFD calculations)
    Joined_Wing.A_engine = np.pi*(0.9525)**2   
    ...
    Joined_Wing.add_component(Wing1)
...
## Need to change all    
    ...
    #Wing4.ar = 8.0
    #Wing4.span =35.66
    #Wing4.sweep=25.0*np.pi/180
    #Wing4.symmetric = True
    #Wing4.t_c = 0.1
    #Wing4.taper= 0.16    

    #wing_planform(Wing4)
    #Wing4.chord_mac= 12.5
    #Wing4.S_exposed=0.8*Wing1.area_wetted
    #Wing4.S_affected=0.6*Wing1.area_wetted 
    ##Wing1.Cl = 0.3
    #Wing4.e = 0.9
    #Wing4.alpha_rc =3.0
    #Wing4.alpha_tc =3.0  
    #Wing4.highlift == False
  
    
    
    #Joined_Wing.add_component(Wing4)    
    ...
    Joined_Wing.add_component(Wing2)
...
#----------------------Vertcal stabalizer----------    
    ...
    Joined_Wing.add_component(Wing3)
...
###-----------propulsion------------------------------------------------------------------------    
    ...
    fuelcell.Fuel = fuel
    Joined_Wing.Energy.Converters['Fuel_Cell'] = fuelcell
    ...
    #Joined_Wing.add_component(Turbofan)
    Joined_Wing.add_component(Ductedfan)
# add a simple aerodynamic model
    ...
    wing_aero.initialize(Joined_Wing,Cl_var)
    Joined_Wing.Aerodynamics = wing_aero   
    ...
    takeoff_config =  Joined_Wing.new_configuration("takeoff")

    # create cruise configuration
    cruise_config =  Joined_Wing.new_configuration("cruise")    
...
#-----------create configurations---------------------------------------------------------
    ...
    flight.tag = "Joined_Wing test mission"
    flight.m0 = Joined_Wing.Mass_Props.m_full          # kg
